                    ...
                    <form class="w-full border border-t-0 rounded-b p-8" action="{{ route('register') }}" method="POST">
                        ...
                        <div class="block mb-4">
                            <div class="relative">
                                ...
                                <input autofocus required type="text" name="username" placeholder="Username" value="{{ old('username') }}" class="block appearance-none outline-none w-full h-full border focus:border-blue bg-grey-lightest text-grey-darker py-3 pr-3 pl-9 rounded{{ $errors->first('username', ' border-red') }}" />
                            </div>

                            @if ($errors->has('username'))
                                <div class="mt-2 text-sm font-semibold text-red-light">{{ $errors->first('username') }}</div>
                            @endif
                        </div>
